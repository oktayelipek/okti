...
class RuleItem:
    ...
    source_type: str  # "cursor", "cline", "copilot", "agents", "claude", "oktigent"
    path: str
    title: str
    content: str
...
def load_universal_rules(workspace: Path | None = None) -> list[RuleItem]:
    ...
    ws = workspace or Path(os.environ.get("OKTIGENT_WORKSPACE", Path.cwd()))
    rules: list[RuleItem] = []
    ...
    # 6. Oktigent memory & local rules: .oktigent/rules/*.md, .oktigent/memory.md
    oktigent_rules_dir = ws / ".oktigent" / "rules"
    if oktigent_rules_dir.is_dir():
        for r_file in sorted(oktigent_rules_dir.glob("*.md")):
            try:
                content = r_file.read_text(encoding="utf-8").strip()
                if content:
                    rel = str(r_file.relative_to(ws))
                    rules.append(RuleItem(
                        source_type="oktigent",
                        path=rel,
                        title=f"Oktigent Rule ({r_file.stem})",
                        content=content,
                    ))
            except Exception as e:
                logger.debug("Failed reading oktigent rule %s: %s", r_file, e)

    oktigent_mem = ws / ".oktigent" / "memory.md"
    if oktigent_mem.is_file():
        try:
            content = oktigent_mem.read_text(encoding="utf-8").strip()
            if content:
                rules.append(RuleItem(
                    source_type="oktigent_memory",
                    path=".oktigent/memory.md",
                    title="Oktigent Project Memory",
                    content=content,
                ))
        except Exception as e:
            logger.debug("Failed reading oktigent memory: %s", e)
